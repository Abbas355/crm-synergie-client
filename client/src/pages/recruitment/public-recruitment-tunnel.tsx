...
const SignatureModal = ({ documentId, documentName, isOpen, onClose }: SignatureModalProps) => {
  ...
  const signDocumentMutation = useMutation({
    mutationFn: async ({ documentId, signatureData }: { documentId: number, signatureData: { signature: string } }) => {
      const res = await apiRequest("POST", `/api/recruitment/public/documents/sign`, { 
        documentId,
        signatureData
      });
    ...
    onSuccess: (data) => {
      queryClient.invalidateQueries({ 
        queryKey: ['/api/recruitment/public/prospects/current'] 
      });
      
      // Invalider la liste des documents
      if (data.document?.prospectId) {
        queryClient.invalidateQueries({ 
          queryKey: [`/api/recruitment/public/prospects/${data.document.prospectId}/documents`] 
        });
      }
...
const EmailModal = ({ documentId, documentName, isOpen, onClose }: EmailModalProps) => {
  ...
  const sendEmailMutation = useMutation({
    mutationFn: async ({ documentId, recipientEmail, recipientName }: { documentId: number, recipientEmail: string, recipientName?: string }) => {
      const res = await apiRequest("POST", `/api/recruitment/public/documents/email`, { 
        documentId, 
        recipientEmail,
        recipientName
      });
    ...
    onSuccess: () => {
      toast({
        title: "Email envoyé",
        description: "Le document a été envoyé avec succès à l'adresse fournie.",
      });
      setEmail('');
      setName('');
      onClose();
...
function DocumentList({ prospectId, type }: DocumentListProps) {
  const [signatureModal, setSignatureModal] = useState({ isOpen: false, documentId: 0, documentName: '' });
  const [emailModal, setEmailModal] = useState({ isOpen: false, documentId: 0, documentName: '' });
  
  const { data: documents, isLoading } = useQuery<RecruitmentDocument[]>({
    queryKey: [`/api/recruitment/public/prospects/${prospectId}/documents`, type],
    queryFn: async () => {
      const url = type 
        ? `/api/recruitment/public/prospects/${prospectId}/documents?type=${type}`
        : `/api/recruitment/public/prospects/${prospectId}/documents`;
  ...
  // Ouvrir la modal de signature
  const openSignatureModal = (documentId: number, documentName: string) => {
    setSignatureModal({ isOpen: true, documentId, documentName });
  };
  
  // Fermer la modal de signature
  const closeSignatureModal = () => {
    setSignatureModal({ isOpen: false, documentId: 0, documentName: '' });
  };
  
  // Ouvrir la modal d'email
  const openEmailModal = (documentId: number, documentName: string) => {
    setEmailModal({ isOpen: true, documentId, documentName });
  };
  
  // Fermer la modal d'email
  const closeEmailModal = () => {
    setEmailModal({ isOpen: false, documentId: 0, documentName: '' });
  };
        ...
        <div key={doc.id} className="border rounded-md p-3 flex justify-between items-center">
          <div className="flex items-center flex-1 min-w-0">
            ...
            <div className="truncate">
              <div className="font-medium truncate">{doc.nomFichier}</div>
              <div className="text-xs text-muted-foreground">
                {doc.estSigne ? "Signé" : "En attente de signature"} • {new Date(doc.createdAt).toLocaleDateString()}
              </div>
            </div>
          </div>
...
// Composant principal du tunnel de recrutement public
export default function PublicRecruitmentTunnel() {
  ...
  useEffect(() => {
    ...
    const prospectIdParam = params.get('id');
    const tokenParam = params.get('token');
    
    if (prospectIdParam && tokenParam) {
      setProspectId(parseInt(prospectIdParam));
      setToken(tokenParam);
    }
  }, []);

  // Récupérer les données du prospect avec l'ID et le token
  const { data: prospectData, isLoading } = useQuery<RecruitmentProspect>({
    queryKey: [`/api/recruitment/public/prospects/${prospectId}`],
    queryFn: async () => {
      const res = await apiRequest("GET", `/api/recruitment/public/prospects/${prospectId}?token=${token}`);
      return await res.json();
    },
    enabled: !!prospectId && !!token,
  });
  ...
  const updateFormationStatus = useMutation({
    mutationFn: async (data: { formationCompletee: boolean }) => {
      const res = await apiRequest("PUT", `/api/recruitment/public/prospects/${prospectId}/formation-status?token=${token}`, data);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/recruitment/public/prospects/${prospectId}`] });
      toast({
  ...
  const submitFormMutation = useMutation({
    mutationFn: async (formData: FormData) => {
      ...
      const res = await apiRequest("POST", `/api/recruitment/public/prospects/${prospectId}/submit-form`, formData);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/recruitment/public/prospects/${prospectId}`] });
  ...
  const generateContractMutation = useMutation({
    mutationFn: async () => {
      if (!prospectId) throw new Error("Données prospect manquantes");
      const res = await apiRequest("POST", `/api/recruitment/public/prospects/${prospectId}/generate-contract?token=${token}`, {});
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ 
      ...
      queryClient.invalidateQueries({ 
        queryKey: [`/api/recruitment/public/prospects/${prospectId}/documents`] 
      });
      toast({
  ...
  const generateAttestationMutation = useMutation({
    mutationFn: async () => {
      if (!prospectId) throw new Error("Données prospect manquantes");
      const res = await apiRequest("POST", `/api/recruitment/public/prospects/${prospectId}/generate-attestation?token=${token}`, {});
      return await res.json();
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ 
  ...
  const submitContractMutation = useMutation({
    mutationFn: async () => {
      if (!contratSigne || !prospectId) throw new Error("Contrat manquant");
      
      const formData = new FormData();
      formData.append("contratSigne", contratSigne);
      formData.append("token", token || '');
      
      const res = await apiRequest("POST", `/api/recruitment/public/prospects/${prospectId}/submit-contract`, formData);
      return await res.json();
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [`/api/recruitment/public/prospects/${prospectId}`] });
  ...
  // Gérer le changement d'onglet en fonction de la progression
  useEffect(() => {
    if (prospectData) {
  ...
  // Si les paramètres sont manquants, afficher une erreur
  if (!prospectId || !token) {
    return (
      <AppLayout>
        <div className="container">
          <div className="flex flex-col items-center justify-center h-96 gap-4">
            ...
            <h1 className="text-2xl font-semibold text-center">Paramètres manquants</h1>
            <p className="text-center text-muted-foreground">
              L'accès au tunnel de recrutement nécessite des paramètres valides.
                ...
                {prospectData?.contratSigne && (
                  <CardFooter>
                    <AlertSuccess className="w-full">
                      ...
                      <AlertDescription>
                        Votre contrat signé a été reçu avec succès. Félicitations, vous êtes maintenant un vendeur agréé.
                      </AlertDescription>
                    </AlertSuccess>
                  </CardFooter>
                )}
